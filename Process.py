...
def create_fields(opt):
    if opt.load_weights is None:
        ...
        opt.tok_src = tokenize(opt.src_lang)
        opt.tok_trg = tokenize(opt.trg_lang)

        TRG = data.Field(lower=True, tokenize=opt.tok_trg.tokenizer, init_token='<sos>', eos_token='<eos>', pad_token="<pad>")
        SRC = data.Field(lower=True, tokenize=opt.tok_src.tokenizer, pad_token="<pad>")
